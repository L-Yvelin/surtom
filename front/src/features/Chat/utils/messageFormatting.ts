...
export function isPrivateMessage(message: Server.ChatMessage.Type): message is Server.ChatMessage.Text {
  return message.type === Server.MessageType.PRIVATE_MESSAGE;
}

export function isScoreMessage(message: Server.ChatMessage.Type): message is Server.ChatMessage.Score {
...
export function isSavedChatMessage(message: Server.ChatMessage.Type): message is Server.ChatMessage.SavedType {
...
export function extractImageUrls(text: string): URL[] {
  const urlRegex = /https:\/\/[^\s]+?\.(jpg|jpeg|png|webp|avif|gif|svg)(\?[^\s]*)?/g;
...
export function extractUrls(text: string): string[] {
  ...
  const matches = text.match(urlRegex);
  return matches ? matches.map((url) => url.replace(TRAILING_PUNCT_RE, '')) : [];
}
