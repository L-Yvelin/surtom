...
const mockedDatabaseService =
  require("../src/services/databaseService").default;
...
describe("initializeConnection", () => {
  ...
  beforeEach(() => {
    ...
    jest
      .spyOn(constants, "default", "get")
      .mockReturnValue({ MAX_MESSAGES_LOADED: 100, funnyNames: ["a", "b"] });
  });
  ...
  it("should call updateUsersList and log new connection", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      expect.stringContaining("New connection:"),
    );
  });
  ...
  it("should send filtered messages to user", async () => {
    ...
    expect(mockSend).toHaveBeenCalledWith(
      expect.stringContaining("GET_MESSAGES"),
    );
  ...
  it("should log error if getMessages throws", async () => {
    ...
    expect(mockError).toHaveBeenCalledWith(
      "Error getting last message timestamp:",
      expect.any(Error),
    );
  });
...
describe("logMessage", () => {
  ...
  it("should log and call fetch with correct params", () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      expect.stringContaining("<TestUser> hello"),
    );
...
describe("handleMessage", () => {
  ...
  beforeEach(() => {
    ...
    mockHandleCommand = jest
      .spyOn(require("../src/handlers/commandHandler"), "handleCommand")
      .mockImplementation(() => {}) as jest.Mock;
    mockHandleChatMessage = jest
      .spyOn(serverModule, "handleChatMessage")
      .mockResolvedValue(undefined) as jest.Mock;
    mockHandleDeleteMessage = jest
      .spyOn(serverModule, "handleDeleteMessage")
      .mockResolvedValue(undefined) as jest.Mock;
    mockSendMessagesAll = jest
      .spyOn(serverModule, "sendMessagesAll")
      .mockImplementation(() => {}) as jest.Mock;
    mockHandleCustomMessageType = jest
      .spyOn(serverModule, "handleCustomMessageType")
      .mockImplementation(() => {}) as jest.Mock;
    jest.spyOn(console, "log").mockImplementation(() => {});
  });
  afterEach(() => {
...
describe("handleChatMessage", () => {
  ...
  beforeEach(() => {
    ...
    mockHandleScoreToChat = jest
      .spyOn(serverModule, "handleScoreToChat")
      .mockResolvedValue(undefined) as jest.Mock;
    mockHandleMailAll = jest
      .spyOn(serverModule, "handleMailAll")
      .mockResolvedValue(undefined) as jest.Mock;
    jest
      .spyOn(require("../src/utils/helpers"), "validateText")
...
describe("handleScoreToChat", () => {
  ...
  it("should log error if saveMessage fails", async () => {
    ...
    expect(mockError).toHaveBeenCalledWith(
      expect.stringContaining("Failed to save score message"),
    );
  });
  it("should log error if saveMessage throws", async () => {
    mockedDatabaseService.saveMessage.mockRejectedValue(new Error("fail"));
...
describe("handleDeleteMessage", () => {
  ...
  it("should send success and broadcast if deleted", async () => {
    ...
    await handleDeleteMessage(user, 1);
    expect(mockSend).toHaveBeenCalledWith(
      expect.stringContaining("Successfully deleted message"),
    );
  });
...
describe("handleCustomMessageType", () => {
  ...
  it("should send to all listeners if type matches", () => {
    ...
    expect(mockSend).toHaveBeenCalledWith(
      JSON.stringify({ type: "customType", content: { foo: "bar" } }),
    );
  });
  it("should log if no listeners", () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      expect.stringContaining("Wrong message type or empty"),
    );
  });
});
...
describe("sendMessagesAll", () => {
  it("should send message to all open clients", () => {
    ...
    expect(mockClient.send).toHaveBeenCalledWith(
      JSON.stringify({ type: "test", content: "hi" }),
    );
  });
});
