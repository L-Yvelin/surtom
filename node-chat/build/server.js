...
(function (MessageType) {
    MessageType["getMessages"] = "getMessages";
    MessageType["mailAll"] = "mailAll";
    MessageType["scoreToChat"] = "scoreToChat";
    MessageType["deleteMessage"] = "deleteMessage";
    MessageType["isTyping"] = "isTyping";
    MessageType["ping"] = "ping";
})(MessageType || (exports.MessageType = MessageType = {}));
(0, commandHandler_1.subscribe)("updateUsersList", updateUsersList);
console.log("SERVER STARTING...");
const wss = new ws_1.default.Server({ port: 27020 });
wss.on("connection", (connection, req) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (sessionHash)
            sessionUser = (0, helpers_1.mapDatabaseUserToMemoryUser)(yield databaseService_1.default.getUserWithSessionHash(sessionHash));
        const ip = (_c = (req.headers["x-forwarded-for"] || req.socket.remoteAddress)) === null || _c === void 0 ? void 0 : _c.toString();
        const user = new User_1.default((0, helpers_1.generateRandomHash)(), (sessionUser === null || sessionUser === void 0 ? void 0 : sessionUser.name) || (0, helpers_1.getRandomFunnyName)(), (sessionUser === null || sessionUser === void 0 ? void 0 : sessionUser.isModerator) || 0, connection, ip, sessionUser ? true : false, usesMobileDevice);
        if (sessionUser) {
            if (sessionUser.banned) {
                (0, helpers_1.handleIsBanned)(user);
                return;
            }
            else {
        ...
        user.connection.send(JSON.stringify({
            ...
            Texte: {
                ...
                mots: user.isLoggedIn
                    ? yield databaseService_1.default.getDailyScore(user.name)
                    : [],
            },
        }));
        user.connection.send(JSON.stringify({
            Type: "stats",
            Texte: JSON.stringify(yield databaseService_1.default.getScoreDistribution(user.name)),
        }));
        const currentState = store_1.store.getState();
        currentState.users[user.id] = user;
        store_1.store.setState(currentState);
...
function initializeConnection(user) {
    ...
    databaseService_1.default
        .getMessages(!!user.isModerator, constants_1.default.MAX_MESSAGES_LOADED, !user.isLoggedIn)
        .then((messages) => {
        if (messages) {
            user.connection.send(JSON.stringify({
                Type: "getMessages",
...
function handleMessage(user, message) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if ((messageText === null || messageText === void 0 ? void 0 : messageText.length) && messageText.startsWith("/")) {
            ...
            (0, commandHandler_1.handleCommand)(user, command);
            return;
        }
        if (messageType === "getMessages") {
            ...
            if ((!user.isModerator && !(0, helpers_1.validateText)(messageText)) ||
                !(0, helpers_1.validateUsername)(user.name) ||
                (messageImage && messageImage.size > 110 * 1024))
                return;
            try {
                const message = yield databaseService_1.default.saveMessage(user.name, messageText, !!isModerator, "message", messageImage, answeringId);
            ...
            !user.sentTheScore) {
            const motDuJour = yield databaseService_1.default.getTodaysWord();
            ...
            if (decodedJson && decodedJson.tab_couleurs && decodedJson.liste_mots) {
                ...
                try {
                    const message = yield databaseService_1.default.saveMessage(user.name, mots, !!user.isModerator, "score");
            ...
            const listeningTypes = Object.values(store_1.store.getState().users).reduce((acc, user) => {
